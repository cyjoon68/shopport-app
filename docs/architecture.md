...
GraphQL이 조회·일반 mutation 계약을 담당하고 `/v1/ai/chat`은 TanStack AI NDJSON stream을 담당합니다. 별도 BFF는 없습니다. `threadId`는 conversation ID, `runId`는 idempotency key입니다. PostgreSQL stream event는 1시간 보관됩니다.
...
지원 기준은 iOS 18.0+, Android 10(API 29)+, 휴대폰·태블릿입니다. 한국어·KRW만 지원합니다.
