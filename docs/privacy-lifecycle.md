...
- AI 요청의 prompt와 정규화 이미지는 Command Code Provider API에 `x-cmd-zdr: 1`로 전송합니다. ZDR 가능 upstream이 없으면 비보존 경로로 fallback하지 않고 요청을 실패시킵니다.
...
- 계정 삭제 요청 즉시 access를 차단하고 Aurora, OpenSearch 사용자 문서, S3 asset/archive를 비동기 purge합니다.
