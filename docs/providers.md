...
`CatalogProvider`는 `LIVE_QUERY`와 `LICENSED_FEED` capability, `search`, `getProduct`, `resolveOutboundLink`, optional `syncCatalog`을 정의합니다. 계약이 허용한 데이터만 PostgreSQL/OpenSearch/S3에 저장합니다.
