...
승인 문서, credential, rate limit, attribution, 보존·이미지 권한, outbound domain allowlist가 확인되기 전에는 adapter를 등록하거나 production UI에 노출하지 않습니다. HTML parsing, crawling, 비공식 endpoint는 금지합니다. 현재 production은 승인 adapter가 없으면 fail-closed합니다. 개발·테스트만 deterministic fake provider를 사용합니다.
