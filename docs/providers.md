...
`CatalogProvider`는 `LIVE_QUERY`와 `LICENSED_FEED` capability 및 승인된 `search`를 정의합니다. 검색 결과는 PostgreSQL catalog snapshot에 저장되고, 이후 ID 조회는 이 durable snapshot만 사용합니다. 계약이 허용한 데이터만 PostgreSQL/OpenSearch/S3에 저장합니다.
