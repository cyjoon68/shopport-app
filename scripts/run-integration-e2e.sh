...
export API_PORT COMPOSE_PROGRESS COMPOSE_PROJECT_NAME POSTGRES_PORT
export SHOPPORT_API_URL

if ! node --env-file=shopport-be/.env -e 'process.exit(process.env.PROVIDER_API_KEY ? 0 : 1)' || [ -z "${KAKAO_IDENTITY_TOKEN:-}" ] || [ -z "${KAKAO_IDENTITY_NONCE:-}" ]; then
  echo "Integration E2E requires AI provider and Kakao credentials" >&2
  exit 1
fi
